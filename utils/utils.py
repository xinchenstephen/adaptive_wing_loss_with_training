...
def show_landmarks(image, heatmap, gt_landmarks, gt_heatmap):
    ...
    plt.imshow(image)
    plt.scatter(gt_landmarks[:, 0], gt_landmarks[:, 1], s=4, marker='.', c='g')
    plt.scatter(pred_landmarks[:, 0], pred_landmarks[:, 1], s=4, marker='.', c='r')
    plt.show()
...
def get_preds_fromhm(hm, center=None, scale=None, rot=0):
    ...
    preds_orig = np.zeros(preds.size())
...
def save_checkpoint(states, predictions, is_best,
    ...
    if is_best and 'state_dict' in states.keys():
        #这里改变储存方式,防止usrwarning
        torch.save(states['state_dict'],os.path.join(output_dir, 'model_best.pth'))


def create_logger(output_dir, log_dir, phase='train'):
    ...
    tensorboard_log_dir = log_dir /  time.strftime('%Y-%m-%d-%H-%M')
    print('=> creating {}'.format(tensorboard_log_dir))
    tensorboard_log_dir.mkdir(parents=True, exist_ok=True)

    return logger, str(output_dir), str(tensorboard_log_dir)
