...
def eval_model(model, dataloaders, dataset_sizes,
               writer_dict, logger,use_gpu=True, dataset='val',
    ...
    with torch.no_grad():
        for data in dataloaders[dataset]:
            ...
            for i in range(inputs.shape[0]):
                ...
                pred_landmarks, pred_landmarks_oripic = get_preds_fromhm(pred_heatmap.unsqueeze(0))
        ...
        msg = 'val_NME: {:.6f} Failure( > 0.08) Rate: {:.6f} Failure( > 0.10) Rate: {:.6f} Total Count: {:.6f} '.format(global_nme, fail_count_008/total_count, fail_count_010/total_count, total_count)
        logger.info(msg)
