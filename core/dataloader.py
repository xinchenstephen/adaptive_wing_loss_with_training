...
from utils.utils import cv_crop, cv_rotate, draw_gaussian, transform, power_transform, shuffle_lr, fig2data, generate_weight_map, fliplr_joints
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        idx,center, scale,image, heatmap, landmarks, boundary, weight_map= sample["index"],sample["center"], sample["scale"], sample['image'], sample['heatmap'], sample['landmarks'], sample['boundary'], sample['weight_map']
...
class FaceLandmarksDataset_dir(Dataset):
...
class FaceLandmarksDataset(Dataset):
    ...
    def __init__(self, img_root, data_path, data_para,num_landmarks=68, gray_scale=False,
                 detect_face=False, enhance=False, center_shift=0,
        ...
        self.enhance = enhance
        self.center_shift = center_shift
        self.scale_factor = data_para["scale_factor"]
        self.rot_factor = data_para["rot_factor"]
        self.flip = data_para["if_flip"]
        if self.detect_face:
            self.face_detector = MTCNN(thresh=[0.5, 0.6, 0.7])
...
def get_dataset(img_root, data_path, batch_size,data_para,
                num_landmarks=68, phase = "train", rotation=0, scale=0,
    ...
    img_transforms = transforms.Compose([AddBoundary(num_landmarks),
                                         AddWeightMap(),
                                         ToTensor()])

    img_dataset = FaceLandmarksDataset(img_root, data_path,data_para,
    ...
    img_dataloader = torch.utils.data.DataLoader(img_dataset,
                                                   batch_size=batch_size,
                                                   shuffle=False,
                                                   num_workers=6)
                                                   
    data_loaders = {phase: img_dataloader}
    dataset_sizes = {}
    dataset_sizes[phase]= len(img_dataset)
