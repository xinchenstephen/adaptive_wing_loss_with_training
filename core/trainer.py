...
import numpy as np
from utils.utils import fan_NME, show_landmarks, get_preds_fromhm,save_checkpoint
...
def train_model(model, train_dataloaders, train_dataset_sizes, valid_dataloaders,valid_dataset_sizes, batch_size,
                optimizer, criterion, lr_scheduler,
               writer_dict, logger, use_gpu=True, epoches=5, dataset='train',
    ...
    for epoch in range(epoches):
        ...
        for data in train_dataloaders[dataset]:
            ...
            if step % 50 == 0:
                ...
                msg = "epoch {} Step [{} / {}] step Time: {:.6f} loss: {:.4f} ". format(epoch, step, int(train_dataset_sizes['train']/batch_size), step_end - step_start, loss.item())
                logger.info(msg)
        ...
        #每个epoch检验一次
        nme,predictions = eval_model(model,valid_dataloaders,valid_dataset_sizes,writer_dict, logger)
        ...
        save_checkpoint(  
             ...
             }, predictions, is_best, save_path, 'checkpoint_{}.pth'.format(epoch))

        nme_save_path = os.path.join(save_path, 'nme_log.npy')
        np.save(nme_save_path, np.array(nmes))
        msg = 'epoch {}  TRAIN NME: {:.6f} Failure Rate: {:.6f} Total Count: {:.6f} Fail Count: {:.6f}'.format(epoch, epoch_nme, fail_count/total_count, total_count, fail_count)
        logger.info(msg)
        print("is_best", is_best)

    msg = 'training done! Average NME: {:.6f}'.format(global_nme/epoches)
    logger.info(msg)
    msg = 'Everage runtime for a single batch: {:.6f}'.format(total_runtime/run_count)
    logger.info(msg)
    return model
